    ...
    <form @submit.prevent="createProduct">
      <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
        ...
        <div class="sm:col-span-3">
          ...
          <input v-model="product.price" type="number" step="0.01" class="w-full p-2 border rounded" required />
        </div>
        <!-- Initial Stock Quantity -->
        <div class="sm:col-span-3">
          <label class="block text-lg font-medium text-gray-700">Initial Stock Quantity</label>
          <input v-model="product.initial_stock_quantity" type="number" class="w-full p-2 border rounded" required />
        </div>
    
      </div>
      <div class="flex justify-center mt-8">
        <button type="button" @click="router.push('/products')" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Cancel</button>
        <button type="submit" :disabled="loading" class="bg-blue-600 text-white px-6 py-3 rounded-md hover:bg-blue-700 disabled:opacity-50 ml-4">
